...
$query1 = "SELECT class FROM user2 WHERE email='$email' ";
			  ...
			  <div class="box">
				...
				<div class="box-body">
					...
					<div class="form-group col-md-4">
						...
						<div>
							<select class="form-control" name="class" id="classDropdown" required onchange="fetchBooks()">
                            <option disabled selected>Please Select Class/Course</option>
                            <?php
                                if ($result1->num_rows > 0) {
                                    while ($row1 = $result1->fetch_assoc()) {
                                        // Assuming 'class' is a JSON encoded array
                                        $classes = json_decode($row1['class'], true); // Decode JSON to an associative array
					...
					<div class="form-group col-md-3 d-none">
						...
						<div>
						 <input class="form-control" id="subject" name="subject" value="<?php echo $subject;?>" required>
                        </div>
					</div>
					<div class="form-group col-md-4">
						...
						<div>
							<select class="form-control" name="book" id="book" required>
							  <option disabled selected>Please Select Book</option>
							   <?php include"ajax/fetch_books.php";?>
							 </select>
						</div>
					</div>
					
					<input class="form-control" type="hidden" name="papertitle" id="papertitle" value="<?php echo $_GET['papertitle'];?>" required>
...
function fetchBooks() {
    ...
    var subjectName = document.getElementById("subject").value; // Assuming subject is already set

    if (className && subjectName) {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", "ajax/fetch_books.php?class=" + encodeURIComponent(className) + "&subject=" + encodeURIComponent(subjectName), true);
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                var bookDropdown = document.getElementById("book");
                bookDropdown.innerHTML = xhr.responseText; // Update the book dropdown with the response
            }
        };
        xhr.send();
    }
            ...
            success: function(response) {
                try {
                    ...
                    if (result.error) {
                        ...
                        var selectedBook1 = $('#book').val();
                        var selectedSubject1 = $('#subject').val();
                        var selectedClass1 = $('#classDropdown').val();
                        var redirectUrl = 'question-paper.php?book=' + encodeURIComponent(selectedBook1) +
                                          '&class=' + encodeURIComponent(selectedClass1) +
                                          '&subject=' + encodeURIComponent(selectedSubject1) +
                                          '&papertitle=' + encodeURIComponent(paperTitle);
