...
$sql = "SELECT * FROM mcq WHERE id='$updateid'"; // Fetching id along with questions
...
<div class="wrapper">
    ...
    for ($index = 0; $index < $totalQuestions; $index++) {
        ...
        echo '<div class="">';
        echo '<input class="form-control" type="text" name="answer[]" value="'.$answersArray[$index]. '" placeholder="' . htmlspecialchars($answersArray[$index] ?? '') . '">';
        echo '</div>';
        echo '</div>';
     ...
     $('#edit-questions').on('submit', function(event) {
        ...
        if (!isValid) {
            ...
            return; // Stop the submission
                ...
                $.ajax({
                    url: 'ajax/edit-mcq.php', // The PHP script to handle the insertion
                    type: 'POST',
                    ...
                    success: function(response) {
                        ...
                        window.location.href = 'view-mcqquestions.php';
                    },
