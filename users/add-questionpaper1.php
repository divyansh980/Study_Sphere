...
<div class="content-wrapper">
			  ...
			  <div class="box">
				...
				<div class="box-body">
				    ...
				    <div class="form-group col-md-4">
						...
						<div>
							<input class="form-control" name="institutionname" id="institutionname" required>
						</div>
					</div>
					<div class="form-group col-md-4">
						<label class="col-form-label">Address Of Institution</label>
						<div>
							<input class="form-control" name="institutionaddress" id="institutionaddress" required>
						</div>
					</div>
					<div class="form-group col-md-4">
					...
					<button id="fetchQuestions" class=" btn btn-primary"  type="button">Show Questions</button>
					<div class="form-group col-md-12">
    ...
    <input type="text" id="totalMarks" class="form-control" value="0" readonly>
</div>
            ...
            if (response.questions && response.questions.length > 0) {
                ...
                response.questions.forEach(function(questionData, index) {
                    ...
                    questionsContainer.append('<div class="form-check">' +
                        '<input type="checkbox" value="' + encodeURIComponent(concatenatedQuestions) + '" id="question_' + index + '" name="selected_questions[]" class="form-check-input" data-marks="' + marks + '">' +
...
function calculateTotalMarks() {
        var totalMarks = 0; // Initialize total marks
        $('input[name="selected_questions[]"]:checked').each(function() {
            totalMarks += parseInt($(this).data('marks')); // Add the marks of the selected question
        });
        $('#totalMarks').val(totalMarks); // Update the total marks input field
    }

    // Event listener for question checkboxes
    $(document).on('change', 'input[name="selected_questions[]"]', function() {
        calculateTotalMarks(); // Recalculate total marks on checkbox change
    });
